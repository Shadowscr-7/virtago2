...
export const productApi = {
  ...
  getProducts: (params?: { 
    ...
    brand?: string
    search?: string
  }) => get(`/products${params ? '?' + new URLSearchParams(params as Record<string, string>).toString() : ''}`),
...
export const locationApi = {
  ...
  // Geocodificación
  geocode: (address: string) => get(`/location/geocode?address=${encodeURIComponent(address)}`),
...
export const orderApi = {
  ...
  getOrders: (params?: { page?: number; limit?: number }) => 
    get(`/orders${params ? '?' + new URLSearchParams(params as Record<string, string>).toString() : ''}`),
...
export const favoritesApi = {
  ...
  removeFromFavorites: (productId: string) => patch('/favorites/remove', { productId }),
}
