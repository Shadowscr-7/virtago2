...
export function ImageUploadZone({ 
  ...
  const validFilesCount = filePreviews.filter(p => p.status === 'pending').length
        ...
        <div className="relative z-10 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 text-sm">
              Formatos soportados: {acceptedFormats.map(f => f.split('/')[1].toUpperCase()).join(', ')} • 
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    {validFilesCount} válido{validFilesCount !== 1 ? 's' : ''} • {formatFileSize(totalSize)}
                  </p>
                </div>
              </div>
                ...
                >
                  <div className="aspect-square rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-700 relative">
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center ${
                      ...
                      {preview.status === 'error' && <AlertCircle className="w-6 h-6 text-white" />}
                      {preview.status === 'uploading' && <Loader2 className="w-6 h-6 text-white animate-spin" />}
                      {preview.status === 'success' && <CheckCircle className="w-6 h-6 text-white" />}
