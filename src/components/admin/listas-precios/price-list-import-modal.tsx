...
import { api, PriceListBulkData, PriceListBulkCreateResponse } from '@/api';
...
export function PriceListImportModal({ isOpen, onClose, onSuccess }: PriceListImportModalProps) {
  ...
  const transformToAPIFormat = (data: Record<string, unknown>[]): PriceListBulkData[] => {
    return data.map(row => ({
      price_list_id: getValue(row, 'price_list_id') || getValue(row, 'priceListId'),
      name: getValue(row, 'name'),
      currency: getValue(row, 'currency', 'USD'),
      country: getValue(row, 'country', 'UY'),
      customer_type: getValue(row, 'customer_type') || getValue(row, 'customerType', 'all'),
      channel: getValue(row, 'channel', 'all'),
      start_date: getValue(row, 'start_date') || getValue(row, 'validFrom') || new Date().toISOString().split('T')[0],
      description: getValue(row, 'description', '') || undefined,
      end_date: getValue(row, 'end_date') || getValue(row, 'validTo') || undefined,
  ...
  const validateJSON = (jsonString: string): { isValid: boolean; data?: Record<string, unknown>[]; errors: string[] } => {
    ...
    try {
      ...
      parsed.forEach((priceList: Record<string, unknown>, index: number) => {
        if (!priceList.name) errors.push(`Lista ${index + 1}: El nombre es requerido`);
        if (!priceList.priceListId && !priceList.price_list_id) errors.push(`Lista ${index + 1}: El priceListId es requerido`);
      });
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log(`[PriceListImport] Archivo parseado: ${rawData.length} filas, ${columns.size} columnas:`, columnList);

      try {
        const schemaHeaders: Record<string, string> = {};
        if (token) schemaHeaders['Authorization'] = `Bearer ${token}`;
        const schemaResponse = await fetch('/api/imports/schema/priceLists', { headers: schemaHeaders });
        ...
        if (schemaResponse.ok) {
          ...
          console.log(`[PriceListImport] Auto-match: ${allRequiredMapped ? 'TODAS las columnas obligatorias coinciden' : 'Faltan columnas obligatorias'}`, mapping);
        }
      } catch (schemaErr) {
        console.warn('[PriceListImport] No se pudo verificar esquema:', schemaErr);
        setColumnsMatch(true);
      }
    } catch (err) {
      console.error('[PriceListImport] Error parseando archivo:', err);
  ...
  const handleDownloadExample = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/listprice/format', { method: 'GET', headers });
      ...
      const fileName = contentDisposition
        ...
        : 'ListasPrecios_Plantilla_Importacion.xlsx';
  ...
  const processImportResponse = (response: PriceListBulkCreateResponse) => {
    const results: ImportResultRow[] = [];

    if (response.results.priceLists) {
      response.results.priceLists.forEach((pl) => {
        results.push({ name: pl.name || '—', priceListId: pl.price_list_id || '—', status: 'success' });
      });
    }

    if (response.results.errors) {
      response.results.errors.forEach((err) => {
        results.push({ name: err.priceList?.name || '—', priceListId: err.priceList?.price_list_id || '—', status: 'error', message: err.error });
      });
    }
  ...
  const handleImport = async () => {
    ...
    try {
      if (uploadMethod === 'json') {
        ...
        const apiResponse = await api.admin.priceLists.bulkCreate(dataForAPI);

        if (apiResponse.success) {
          const response = apiResponse.data as PriceListBulkCreateResponse;
          processImportResponse(response);
          if (response.results.errorCount > 0) {
            showToast({ title: "Importación parcial", description: `${response.results.successCount} listas importadas, ${response.results.errorCount} con errores`, type: "warning" });
          } else {
            showToast({ title: "Importación exitosa", description: `${response.results.successCount} listas importadas correctamente`, type: "success" });
          }
          onSuccess?.();
        } else {
          throw new Error(apiResponse.message || 'Error al importar listas de precios');
        }
      } else if (uploadMethod === 'file' && uploadedFile) {
        if (columnsMatch && fileRawData.length > 0) {
          console.log('[PRICELIST IMPORT] ✅ Columnas coinciden, importando con mapeo automático...');
          ...
          console.log('[PRICELIST IMPORT] 📁 Enviando archivo directamente...');
          const formData = new FormData();
          formData.append('file', uploadedFile);
          const apiResponse = await api.admin.priceLists.bulkCreate(formData);
          if (apiResponse.success) {
            const response = apiResponse.data as PriceListBulkCreateResponse;
            processImportResponse(response);
            showToast({ title: "Importación exitosa", description: "Las listas de precios se han importado correctamente", type: "success" });
            onSuccess?.();
          } else {
  ...
  const handleMappingConfirm = async (mappedData: Record<string, unknown>[], _mapping: ColumnMapping) => {
    ...
    try {
      console.log(`[PRICELIST IMPORT] 📤 Importando ${mappedData.length} listas con mapeo personalizado...`);
      const dataForAPI = transformToAPIFormat(mappedData);
      const apiResponse = await api.admin.priceLists.bulkCreate(dataForAPI);
      ...
      console.error('[PRICELIST IMPORT] ❌ Error con mapeo:', error);
      showToast({ title: "Error al importar", description: error instanceof Error ? error.message : "Ocurrió un error inesperado", type: "error" });
      setError(error instanceof Error ? error.message : "Error al importar listas de precios");
    } finally {
      setIsProcessing(false);
              ...
              {importResults && importResults.length > 0 ? (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr style={{ backgroundColor: `${themeColors.primary}10` }}>
                        ...
                        <th className="px-4 py-3 text-left font-medium" style={{ color: themeColors.text.primary }}>ID Lista</th>
                    ...
                    <tbody>
                      {importResults.map((row, i) => (
                        <tr key={i} className="border-t" style={{ borderColor: themeColors.primary + '10', backgroundColor: row.status === 'error' ? '#fee2e210' : 'transparent' }}>
                          ...
                          <td className="px-4 py-3" style={{ color: themeColors.text.primary }}>{row.priceListId}</td>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm mt-2" style={{ color: themeColors.text.secondary }}>
                    {importSummary.success} lista(s) importada(s) exitosamente{importSummary.errors > 0 && `, ${importSummary.errors} con errores`}
                  </p>
                </div>
              ) : null}
            </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold" style={{ color: themeColors.text.primary }}>Importar Listas de Precios</h2>
                <p className="text-sm mt-1" style={{ color: themeColors.text.secondary }}>Sube un archivo o pega JSON con los datos de las listas de precios</p>
              </div>
              <button onClick={handleClose} className="p-2 rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-gray-800" style={{ color: themeColors.text.secondary }}><X className="w-6 h-6" /></button>
            </div>
              ...
              {uploadMethod === 'file' ? (
                <div>
                  <div className="border-2 border-dashed rounded-2xl p-8 text-center transition-all hover:border-opacity-80" style={{ borderColor: themeColors.primary + '40' }}>
                    ...
                    <input type="file" accept=".csv,.xlsx,.json" onChange={handleFileChange} className="hidden" id="file-upload-pricelist" />
                    <label htmlFor="file-upload-pricelist" className="inline-block px-6 py-3 rounded-lg cursor-pointer transition-all text-white"
                    ...
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 p-4 rounded-xl border"
                      ...
                      {columnsMatch ? (
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm font-medium" style={{ color: themeColors.text.primary }}>{fileRawData.length} lista(s) de precios lista(s) para importar</p>
                        </div>
                      ) : (
                        <div>
    ...
    "validFrom": "2024-01-01"
                  ...
                  {jsonInput && (
                    <div className="mt-4">
                      {validationErrors.length === 0 ? (
                        <div className="flex items-center gap-2 text-green-600"><CheckCircle className="w-5 h-5" /><span>JSON válido - {uploadedData.length} lista(s) detectada(s)</span></div>
                      ) : (
                        <div>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={handleImport}
                ...
                {isProcessing ? (<><Loader2 className="w-5 h-5 animate-spin" />Procesando...</>) : (<><Upload className="w-5 h-5" />Importar Listas de Precios</>)}
              </button>
            </div>
          </motion.div>
        </motion.div>
